...
const CheakOut = () => {
    ...
    return (
        <form onSubmit={handleClick} className='lg:my-10'>
            ...
            <input name='mobile' type="text" placeholder="Phone" className="input input-bordered w-full " />
            <input name='email' type="text"  disabled value={user?.email} className="input input-bordered w-full " />
            <textarea name='message' className="textarea textarea-bordered w-full h-48" placeholder="Type your"></textarea>
            
            </div>
            <input  type="submit"   className="btn lg:mx-10 px-10" value="Place Your Order" />
        </form>
    );
};
